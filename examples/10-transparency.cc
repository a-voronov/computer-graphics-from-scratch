...
//
// ```
// clang++ -std=c++17 examples/10-transparency.cc -o bin/10-transparency -O3 -fno-fast-math
// bin/10-transparency
// ```

#include "bmp.h"
#include "bvh-transparency.h"
#include <chrono>

const float EPSILON = 0.001;
const bool SUBSAMPLING = false;
const int8_t RAY_TRACING_DEPTH = 4;
...
optional<Vec3> refractRay(const Vec3& ray, float cur_refraction, const Vec3& normal, float next_refraction) {
    // incident ray, so call it I
    Vec3 vec_i = ray.normalized();
    Vec3 vec_n = normal;
    // Make N oppose I; if we’re inside, flip N and swap media
    float cos_in = vec_i.dot(vec_n);
    // this means ray is inside the shape and points the same way (towards exit) as the normal
    if (cos_in > 0.0f) {
        // flipping N so it's against I according to Snell's formula, hence swapping refration indices as well
        vec_n = vec_n * -1.0f;
        std::swap(cur_refraction, next_refraction);
    }
    // we negate cos IN in two cases - when we're inside and outside of the shape:
    // * in case we're outside - cos is negative, so we make it positive according to Snell's formula,
    // * in case we're inside - N and I point towards the same direction, so we flip N and swap refraction indices,
    //   however it means that now we should negate cos IN result as well.
    cos_in = -cos_in;

    float eta = cur_refraction / next_refraction;
    float k = 1.0f - pow(eta, 2) * (1.0f - pow(cos_in, 2));
    if (k < 0.0f) {
        return nullopt;
    }
    return (vec_i * eta + vec_n * (eta * cos_in - sqrt(k)))
        .normalized();
}
...
Color traceRay(const Ray& ray, float refraction, float t_min, float t_max, int8_t recursion_depth, const Scene& scene) {
    if (auto hit = scene.bvh->intersect(ray, t_min, t_max)) {
        Vec3 view = (ray.direction * -1).normalized();
        Color local_color = hit->material->color * computeLighting(hit->point, hit->normal, view, hit->material->specular, scene);

        if (recursion_depth <= 0) {
            return local_color;
        }

        optional<Color> reflection_color;
        optional<Color> refraction_color;
        float reflective = hit->material->reflective;
        float transparent = hit->material->transparent;

        // calculate color for reflective surface
        if (reflective > 0) {
            ...
            Ray reflection_ray = {hit->point + (hit->normal * EPSILON), vec_r};
            reflection_color = traceRay(reflection_ray, refraction, EPSILON, INFINITY, recursion_depth - 1, scene);
        }
        // calculate color for refractive surface
        if (transparent > 0) {
            if (auto vec_r = refractRay(ray.direction, refraction, hit->normal, hit->material->refraction)) {
                float side = (vec_r->dot(hit->normal) >= 0.0f) ? +1.0f : -1.0f;
                Ray refraction_ray = {hit->point + hit->normal * side * EPSILON, *vec_r};
                refraction_color = traceRay(refraction_ray, hit->material->refraction, EPSILON, INFINITY, recursion_depth - 1, scene);
            }
        }
        // normalizing colors, because otherwise result might be oversaturated/overbrightened
        float colors_sum = reflective + transparent;
        if (colors_sum > 1.0f) {
            reflective /= colors_sum;
            transparent /= colors_sum;
        }
        // simple blending of reflection and refraction colors into one
        // as a weighted average of local, reflection and refraction colors
        Color result_color = local_color * (1.0f - reflective - transparent);
        if (reflection_color) {
            result_color = result_color + *reflection_color * reflective;
        }
        if (refraction_color) {
            result_color = result_color + *refraction_color * transparent;
        }
        return result_color;
    }
...
int main() {
    ...
    scene.objects.reserve(11);

    auto red_material = make_shared<Material>(Material{.color = Color{1, 0, 0}, .specular = 500, .reflective = 0.2 });
    ...
    auto cyan_material = make_shared<Material>(Material{
        .color = Color{0, 1, 1},
        .specular = 10, .reflective = 0.3,
        .refraction = 1.1, .transparent = 0.8
    });
    auto magnifying_material = make_shared<Material>(Material{
        .color = Color{1, 1, 1},
        .specular = 10, .reflective = 0,
        .refraction = 1.5, .transparent = 1
    });
    ...
    scene.objects.emplace_back(makePrimitive(
        CSGShape::make(CSGOperation::INTERSECTION, make_shared<Sphere>(Vec3{-2.5, 1.3, -2}, 0.5), make_shared<Sphere>(Vec3{-2.5, 1.3, -1.75}, 0.5)),
        magnifying_material
    ));
    ...
    scene.objects.emplace_back(makePrimitive(make_shared<Sphere>(Vec3{-2.2, -0.5, -6}, 0.5), cyan_material));
    ...
    if (SUBSAMPLING) {
        ...
        for (int32_t x = start_x; x < end_x; x += 2) {
            for(int32_t y = start_y; y < end_y; y += 2) {
                ...
                image.data[image.offset(x, y)] = traceRay(camera_ray, 1, 1, INFINITY, RAY_TRACING_DEPTH, scene);
            }
        }
        ...
        for (int32_t x = -image.size.width / 2; x < image.size.width / 2; x++) {
            for(int32_t y = -image.size.height / 2; y < image.size.height / 2; y++) {
                Vec3 direction = camera.rotation * canvasToViewport(x, y, image.size, scene);
                Ray camera_ray = {camera.position, direction};
                Color color = traceRay(camera_ray, 1, 1, INFINITY, RAY_TRACING_DEPTH, scene);
    ...
    if (SUBSAMPLING) {
        save_image(bmp_image, image.size.width, image.size.height, "results/10-transparency-subsampling.bmp");
    } else {
        save_image(bmp_image, image.size.width, image.size.height, "results/10-transparency.bmp");
    }
