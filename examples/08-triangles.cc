...
struct Triangle : Object {
    ...
    Triangle(const Vec3& a, const Vec3& b, const Vec3& c, float spec, float refl, const Color& col) : a(a), b(b), c(c) {
        specular = spec;
        reflective = refl;
        color = col;
    }
...
int main() {
    ...
    if (SUBSAMPLING) {
        save_image(bmp_image, image.size.width, image.size.height, "results/08-triangles-subsampling.bmp");
    } else {
        save_image(bmp_image, image.size.width, image.size.height, "results/08-triangles.bmp");
    }
